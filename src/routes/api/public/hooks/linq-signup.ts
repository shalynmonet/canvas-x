        ...
        if (isSignup && (phone || chatId)) {
          const text =
            isLifetimeIntent && lifetimeLive
              ? `CanvOps lifetime access — only $44, no renewals ever! Claim it before 11:59pm Chicago time on Sept 14: ${upgradeUrl}`
              : `Welcome to CanvOps! Try it free and set up your first collab here: ${signupUrl}`;
